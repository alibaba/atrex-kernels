...
Never select hardware from the DSL or source language. Probe the available
Agate hardware and select an environment whose vendor and architecture match
the target implemented by the operator variant. Never publish internal
environment labels, and never infer a fallback target from CUDA, Triton, Gluon,
CuTeDSL, or any other implementation technology.
