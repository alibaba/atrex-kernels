"""Fused Q/K bf16 L2 normalization for SM103 Chunk-GDN prefill."""
...
def _stable_cutlass_dsl_version_hash():
    return hashlib.sha256(b"atrex-gdn-sm103-fused-qk-l2norm-v4-dynamic-layout")
...
_SM103A_COMPILE_OPTIONS = (cute.GPUArch("sm_103a"),)


@cute.kernel
def atrex_aka_gdn_fused_qk_l2norm_bf16_kernel(
    mQ: cute.Tensor,
    mK: cute.Tensor,
    mQOut: cute.Tensor,
    mKOut: cute.Tensor,
...
atrex_aka_gdn_fused_qk_l2norm_bf16_kernel.set_name_prefix("atrex_aka")
...
def launch_fused_qk_l2norm_bf16(
    ...
    atrex_aka_gdn_fused_qk_l2norm_bf16_kernel(
...
def fused_qk_l2_normalize_bf16(
    ...
    *,
    ...
    key = (
        "atrex-aka-fused-qk-l2norm-bf16-d128-v4-dynamic-layout",
        int(device_index),
        q.dtype,
        K_DIM,
    )
    with _compiled_cache_lock:
        ...
        if compiled is None:
            with _stable_cutlass_dsl_version():
                compiled = cute.compile[_SM103A_COMPILE_OPTIONS](
