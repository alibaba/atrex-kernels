"""SM120 Chunk-GDN delta-rule CuTeDSL kernel."""
...
from ..common_utils.alpha import AlphaProcessor
from ..common_utils.collective_store_tma import CollectiveStoreTma
from .custom_compile_cache import KeyedCompileMixin, cached_compile
from ..common_utils.collective_inverse_hmma import CollectiveInverse
from ..common_utils.helpers import SM80, round_down
from ..common_utils.schedule import WorkDesc
...
_DELTA_RULE_CACHE_LOCK = threading.Lock()
...
def _cached_tensormaps(
    ...
    # The kernel writes stream-local TMA descriptors into this scratch buffer.
    # Sharing it across concurrently executing streams would create a race.
...
def _compiled_delta_rule_kernel(
    ...
    with _DELTA_RULE_CACHE_LOCK:
        ...
        if compiled is None:
            ...
            delta_rule_kernel.atrex_gdn_delta_rule_sm120_kernel.set_name_prefix(
                "atrex"
            )
    ...
    def maybe_store_checkpoint(
        ...
        if cutlass.const_expr(self.needs_checkpointing):
            if block_end <= seq_len and block_end % checkpoint_every_n_tokens == cutlass.Int32(0):
                ...
                if (
                    checkpoint_idx >= cutlass.Int32(0)
                    and checkpoint_idx < total_checkpoints
                ):
                    ...
                    gCheckpointKV = mCheckpoint[
                        None, None, o_head_idx, checkpoint_idx]
                    self.kv_store(
                        tKVrKV, gCheckpointKV, kv_thr_mma, v_row_offset)
...
def delta_rule_prefill_dsl(
    ...
    cu_seqlens: torch.Tensor,          # (num_seqs+1,) int32/int64
    scale:      float,
    ...
    """Run the fused SM120 prefill kernel.

    ``aligned_full_blocks=True`` is an internal specialization hint and
    requires every sequence length in ``cu_seqlens`` to be a multiple of 64.
    Callers that cannot prove that condition must leave it as ``None`` or
    ``False``.
    """
    ...
    if (
        ...
        raise ValueError(
            "cu_seqlens must be a contiguous int32/int64 tensor on q.device")
    ...
    if needs_checkpointing:
        ...
        if (
            state_checkpoints.device != q.device
            or not state_checkpoints.is_contiguous()
        ):
            raise ValueError(
                "state_checkpoints must be contiguous and on q.device")
        if checkpoint_cu_starts.dtype != torch.int64:
            raise ValueError(
                f"checkpoint_cu_starts must be int64, got {checkpoint_cu_starts.dtype}")
        if (
            checkpoint_cu_starts.device != q.device
            or checkpoint_cu_starts.ndim != 1
            or checkpoint_cu_starts.numel() != num_seqs + 1
            or not checkpoint_cu_starts.is_contiguous()
        ):
            raise ValueError(
                "checkpoint_cu_starts must be contiguous on q.device with "
                "shape [num_seqs + 1], got "
                f"shape={list(checkpoint_cu_starts.shape)}")
        # Do not inspect checkpoint_cu_starts[-1] from Python here. It is a CUDA
        # tensor in serving, and .item() would serialize every GDN layer on the
        # host. Callers construct the offsets and output buffer together; the
        # kernel consumes both with the validated shapes above.
    ...
    cu_cute = from_dlpack(cu_device, assumed_align=8).mark_layout_dynamic()
