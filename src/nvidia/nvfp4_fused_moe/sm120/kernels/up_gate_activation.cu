...
// ============================================================================
// Optimized doActivation — SwiGLU + FP4 requantization
//
// Key optimizations vs original:
//   1. SiLU computed in float, absmax tracked in float (no float→bf16→float trips)
//   2. Precomputed per-row SF base offset (eliminates get_sf_out_offset_128x4)
//   3. Direct fp32_vec_to_e2m1 from computed values (no PackedVec intermediate)
// ============================================================================

// launch_bounds(128, 8) lets the launcher pick 96 threads for Qwen3-MoE
// (inter_size=768 → elems_per_row=96), while keeping the SM occupancy ceiling
// (128*8 = 1024 threads/SM) identical to the legacy (64*16). For inter_size=512
// the launcher still picks 64 threads.
template <bool Interleaved>
__global__ __launch_bounds__(128, 8)
void atrex_doActivationKernel_opt(
    ...
    bool interleaved)
{
    ...
    if (interleaved) launch(atrex_doActivationKernel_opt<true>);
    else             launch(atrex_doActivationKernel_opt<false>);
}
