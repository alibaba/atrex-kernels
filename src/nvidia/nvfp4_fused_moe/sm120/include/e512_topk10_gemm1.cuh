...
#define TASK29_IMPL_NAMESPACE task29_gemm1_small_m_v1
#endif
...
#ifndef TASK29_ROUND_BF16_BOUNDARY
// When enabled, match the materialized BF16 GEMM1 workspace consumed by
// FlashInfer before applying SwiGLU while keeping the value in registers.
#define TASK29_ROUND_BF16_BOUNDARY 0
#endif

#ifndef TASK29_ROUND_POSTACT_BF16_BOUNDARY
// When enabled, match FlashInfer's BF16 SwiGLU output before computing the
// dynamic scale factor and requantizing to FP4.
#define TASK29_ROUND_POSTACT_BF16_BOUNDARY 0
#endif
...
// The shared scale-factor stage keeps the full 128-row swizzled shape. For a
// single-row expert, only the valid four-byte fragment is loaded and the
// unused rows are zero-filled directly in shared memory.
...
__device__ __forceinline__ void stage_sf_a_16(
    uint8_t* smem, const uint8_t* gmem, bool single_row, int sf_lane) {
    if (!single_row) {
        cp_async_cg_16(smem, gmem);
        return;
    }

    // A single expert row contributes four contiguous scale bytes per
    // 64-wide K atom in the 128x4 swizzle. Materialize the remaining rows as
    // zero directly in shared memory so global SF padding is unnecessary.
    *reinterpret_cast<uint4*>(smem) = make_uint4(0, 0, 0, 0);
    if ((sf_lane & 31) == 0) {
        *reinterpret_cast<uint32_t*>(smem) =
            *reinterpret_cast<const uint32_t*>(gmem);
    }
}
...
#pragma unroll
    ...
    auto issue_stage = [&](int kt, int buf) {
        ...
        if (threadIdx.x < 64) {
            stage_sf_a_16(sSF_A + sf_thread_dst_off,
                          sf_thread_src + sf_k_off,
                          expert_next == expert_off + 1, threadIdx.x);
        } else {
...
void atrex_gemm1_grouped_m16_kernel(
    ...
    bool const use_compact_sf =
        PRIMARY_K == 2560 && PRIMARY_N == 640 &&
        __ldg(&expert_first_token_offset[num_experts]) <= 160;
...
__global__ void atrex_gemm1_grouped_m16_fused_act_kernel(
    ...
    bool const use_compact_sf =
        use_shared_sf_staging && PRIMARY_K == 2560 && PRIMARY_N == 640;
...
#pragma unroll
    ...
    auto fused_pair = [&](int gate_ni, int up_ni, int acc_idx) -> float {
        ...
        if constexpr (TASK29_ROUND_BF16_BOUNDARY != 0) {
            gate = __bfloat162float(__float2bfloat16(gate));
            up = __bfloat162float(__float2bfloat16(up));
        }
        float activated = fused_silu(up) * gate;
        if constexpr (TASK29_ROUND_POSTACT_BF16_BOUNDARY != 0) {
...
#pragma unroll
    ...
    // Make the graph-captured workspace reusable without a separate memset
    // node. The next replay sees the same epoch with a zero completion count;
    // eager launches use a new epoch and are reset by ensure_fused_reduce_epoch.
    __syncthreads();
    if (threadIdx.x == 0) {
        atomicExch(&done_states[state_idx], epoch_state);
    }
}
